...
class MonteCarlo:
    ...
    def __init__(self, window, circle_radius, points):
        pygame.init()
        self.screen = pygame.display.set_mode(window, pygame.FULLSCREEN)
    ...
    def display_square(self, circle_radius):
        pygame.draw.rect(self.screen, "White", (((self.window[0] - self.width)/2), (circle_radius + 3)*2), 2)
    ...
    def is_point_in_circle(self):
        ...
        if x**2 + y**2 <= (self.width/2)*(self.height/2):
            return True
        else:
            return False
    ...
    def draw(self, point):
        ...
        if self.is_point_in_circle(point):
            circle_surface.fill((224, 72, 52)) # red colour
        else:
            circle_surface.fill((0, 90, 145)) # blue colour
